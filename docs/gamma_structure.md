...
### ./ui/builder.ui
Is the UI (XML formated) file which is generated by Glade. This file contains the layout
of Gamma main window.


### ./style
This folder contains `.css` files that are used to style Gamma Gtk widgets. Change the theme
by setting the `.css` file name to `style_file` in `./config.py`.


### ./gtksourceview_styles
This folder contains XML files for styling the GtkSource. Change the GtkSource scheme by
setting the file name to `style_scheme` in `./config.py`.


### ./setup.sh
Is used to setup Gamma to the system. It does the following:
-	copy and rename `./bin/icon.svg` to `~/.icons/com.editor.gamma.svg`
-	creates `./bin/com.editor.gamma.desktop` that contains the current full path to `./bin/gamma`
	and copy it to `~/.local/share/applications/`
-	create a symbolic link to gamma sh file in `/usr/bin/gamma`


### ./bin/gamma
Runs `python3 ./gamma.py` but before that it exports any passing arguments to GAMMA_OPEN_FILE
`export GAMMA_OPEN_FILE="$@"`.



### ./plugins/template/template.py
Is not used plugin, but it is an example or a template for you to 
start your plugin. It is good starting point to your plugin.
Copy the folder `template` rename it and rename `template.py` file to
your plugin name, change `self.name` to your plugin name.
...
The class name must be `Plugin`. Do not change the name. The plugins_manager
will pass "app" reference to your plugin. "app" object is defined in `gamma.py`
from `app` reference you can access pretty much everything related to
Gamma (i.e. window, builder, sourceview, and other plugins)

In `activate` method you probably want to uncomment
```
self.signal_handler.key_bindings_to_plugins.append(self) 
```
if your plugin needs key_bindings.


You should not map `on_window_key_press_event` to your plugin. 
The `key_bindings` function will help you by getting the `keyval_name` (i.e. "e", "space", ..)
and other modifiers like `ctrl`, `alt`, and `shift`. Simply uncomment: 
```
self.signal_handler.key_bindings_to_plugins.append(self)   or
self.signal_handler.any_key_press_to_plugins.append(self)
