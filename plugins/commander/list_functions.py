...
# if 
def filter(row, *user_data):
	commanderWindow = user_data[0]
	searchEntry = user_data[1]
	search_text = searchEntry.get_text().lower()
	show = False
	
	# if empty, show all
	if not search_text:
		commanderWindow.selected_first_row = row
		commanderWindow.listbox.select_row(commanderWindow.listbox.get_row_at_index(0))
		commanderWindow.prepare_second_row = commanderWindow.listbox.get_row_at_index(1)
		return True
	...
	# row ref :/
	if not commanderWindow.selected_first_row and show:
		commanderWindow.selected_first_row = row
		commanderWindow.listbox.select_row(row)
	elif not commanderWindow.prepare_second_row and show:
		commanderWindow.prepare_second_row = row
